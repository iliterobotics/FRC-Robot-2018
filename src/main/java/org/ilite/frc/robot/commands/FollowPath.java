...
public class FollowPath implements ICommand {
	...
	public FollowPath(DriveTrain pDrivetrain, Data data, Trajectory pTrajectory, boolean pIsBackwards) {
	...
	public FollowPath(DriveTrain pDrivetrain, Data pData, File pLeftTrajectoryFile, File pRightTrajectoryFile, boolean pIsBackwards) {
		this(pDrivetrain, pData, Pathfinder.readFromCSV(pLeftTrajectoryFile), Pathfinder.readFromCSV(pRightTrajectoryFile), pIsBackwards);
	}
	
	public FollowPath(DriveTrain pDrivetrain, Data data, Trajectory pLeftTrajectory, Trajectory pRightTrajectory, boolean pIsBackwards) {
		this.mDrivetrain = pDrivetrain;
		this.data = data;
		this.mIsBackwards = pIsBackwards;
	...
	public FollowPath(DriveTrain pDrivetrain, Data data, File pTrajectoryFile, boolean pIsBackwards) {
		this(pDrivetrain, data, Pathfinder.readFromCSV(pTrajectoryFile), pIsBackwards);
	}
	
	public FollowPath(DriveTrain pDrivetrain, Data data, boolean pIsBackwards, Segment ... pSegments) {
		this(pDrivetrain, data, new Trajectory(pSegments), pIsBackwards);
	}
	
	public void initialize(double pNow) {
	  System.out.println("Init FollowPath");
		mDrivetrain.setDriveMessage(new DrivetrainMessage(0, 0, DrivetrainMode.Pathfinder, NeutralMode.Brake));
    ...
    mDrivetrain.setProfilingMessage(new DrivetrainProfilingMessage(mLeftFollower, mRightFollower, mIsBackwards));
	}
