...
import org.ilite.frc.robot.sensors.NavX;
...
public class FollowPath implements Command {
	...
	public FollowPath(DriveTrain pDrivetrain, NavX pNavx, Trajectory pTrajectory, boolean pIsBackwards) {
	...
	public FollowPath(DriveTrain pDrivetrain, NavX pNavx, Trajectory pLeftTrajectory, Trajectory pRightTrajectory, boolean pIsBackwards) {
		this.mDrivetrain = pDrivetrain;
		this.mNavx = pNavx;
		this.mIsBackwards = pIsBackwards;
	...
	public FollowPath(DriveTrain pDrivetrain, NavX pNavx, File pTrajectoryFile, boolean pIsBackwards) {
		this(pDrivetrain, pNavx, Pathfinder.readFromCSV(pTrajectoryFile), pIsBackwards);
	}
	
	public FollowPath(DriveTrain pDrivetrain, NavX pNavx, boolean pIsBackwards, Segment ... pSegments) {
		this(pDrivetrain, pNavx, new Trajectory(pSegments), pIsBackwards);
	}
	
	public void initialize() {
		mDrivetrain.changeModes(ControlMode.PercentOutput);
	...
	public boolean update() {
		...
		mActualHeading = (mIsBackwards) ? Pathfinder.boundHalfDegrees(mNavx.getAngle() + 180): mNavx.getAngle();
