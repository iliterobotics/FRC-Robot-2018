...
public class GetAutonomous {
  private final static ILog log = Logger.createLog(GetAutonomous.class);
...
//    parseEntries();
    ...
    while(System.currentTimeMillis() < timerStart + 1) {
	  ...
	  boolean received = parseEntries();
		...
		log.info("RECEIVED: " + mReceivedCubeActionPrefs);
		log.info("SAME SIDE: " + mSameSideCubeActionPrefs);
		log.info("OTHER SIDE: " + mOtherSideCubeActionPrefs);
		log.info("AVAILABLE:" + mAvailableCubeActions);
...
////			log.info("=================== Autonomous chose: " + prefAction.toString());
////			if(mDelay > 15) {
...
//		}
		...
		if(mCommands.isEmpty()) {
		  log.info("====================== COMMAND QUEUE IS EMPTY - CROSSING AUTO LINE");
		  crossAutoLine();
		}
		log.info("ENDING");
		return mCommands;
	...
	public void doScale() {
      ...
      mCommands.add(new Delay(0.5));
//		 //Back up from scale
		 mCommands.add(new DriveStraight(mDriveTrain, mData, AutoDimensions.SAME_SIDE_SCALE_BACK_UP, 0.01).coast());
//		  
//		 // Reset elevator
...
////		  mCommands.add(new IntakeCube(mIntake, mCarriage, 0.7, 5, true));
	...
	public void doSwitch() {
		// TODO replace with turning scalar
		log.info("Doing switch autonomous starting on %s\n", mStartingPos);
		switch (mStartingPos) {
		case LEFT:
		case RIGHT:
  		...
  		mCommands.add(new ElevatorToPosition(mElevator, EElevatorPosition.SECOND_TAPE));
			mCommands.add(new DriveStraight(mDriveTrain, mData, AutoDimensions.SAME_SIDE_SWITCH_TO_SWITCH, 0.05, true)); //changed power from 0.1 to 0.05
			mCommands.add(new ReleaseCube(mCarriage, CarriageState.KICKING, 1));
			mCommands.add(new Delay(1));
			mCommands.add(new DriveStraight(mDriveTrain, mData, AutoDimensions.SAME_SIDE_SWITCH_BACK_UP, 0.01).coast());
...
//			mCommands.add(new DriveStraight(mDriveTrain, mData, AutoDimensions.SAME_SIDE_SWITCH_TO_NULL_ZONE));
		...
		case MIDDLE:
			...
			case LEFT:
			  ...
			  mCommands.add(new DriveStraight(mDriveTrain, mData, 26.0, 0.01));
			  mCommands.add(new HoldPosition(mDriveTrain, mData, HoldType.HOLD_BOTH, 0.0));
			  
			  mCommands.add(new GyroTurn(mDriveTrain, mPigeon, -41, 3));
	      mCommands.add(new HoldPosition(mDriveTrain, mData, HoldType.HOLD_BOTH, 0.0));

			  //98 inches is the distance to the left of the switch
			  mCommands.add(new DriveStraight(mDriveTrain, mData, 98.0, 0.01));
	      mCommands.add(new HoldPosition(mDriveTrain, mData, HoldType.HOLD_BOTH, 0.0));
	      ...
	      mCommands.add(new HoldPosition(mDriveTrain, mData, HoldType.HOLD_BOTH, 0.0));
			  ...
			  mCommands.add(new DriveStraight(mDriveTrain, mData, 40, 0.01));
			  mCommands.add(new ReleaseCube(mCarriage, CarriageState.KICKING, 1));
			  mCommands.add(new ElevatorToPosition(mElevator, EElevatorPosition.BOTTOM));
...
//	      mCommands.add(new HoldPosition(mDriveTrain, mData, HoldType.HOLD_BOTH, 0.0));
	        ...
	        mCommands.add(new HoldPosition(mDriveTrain, mData, HoldType.HOLD_BOTH, 0.0));
				  ...
				  mCommands.add(new GyroTurn(mDriveTrain, mPigeon, 36, 3));
	        mCommands.add(new HoldPosition(mDriveTrain, mData, HoldType.HOLD_BOTH, 0.0));

				  //92 inches is the distance to the left of the switch
				  mCommands.add(new DriveStraight(mDriveTrain, mData, 92.0, 0.01));
	        mCommands.add(new HoldPosition(mDriveTrain, mData, HoldType.HOLD_BOTH, 0.0));
...
//	        mCommands.add(new HoldPosition(mDriveTrain, mData, HoldType.HOLD_BOTH, 0.0));
  ...
  public void doOppositeScale() {
    ...
    case RIGHT:
      ...
      mCommands.add(new ParallelCommand(
          new GyroTurn(mDriveTrain, mPigeon, mTurnScalar * -95d, 5), 
          new ElevatorToPosition(mElevator, EElevatorPosition.THIRD_TAPE)));
      mCommands.add(new HoldPosition(mDriveTrain, mData, HoldPosition.HoldType.HOLD_BOTH, 0.0));
      mCommands.add(new DriveStraight(mDriveTrain, mData, 9, 0.01));
      mCommands.add(new Delay(0.1));
      mCommands.add(new ReleaseCube(mCarriage, CarriageState.KICKING, 1));
      mCommands.add(new Delay(0.5));
      mCommands.add(new DriveStraight(mDriveTrain, mData, -4, 0.01).coast());
      mCommands.add(new ElevatorToPosition(mElevator, EElevatorPosition.BOTTOM));
      break;
    case MIDDLE:
      break;
  ...
  public void doOppositeSwitch() {
    // TODO replace with turning scalar
    log.info("Doing opposite switch autonomous starting on %s\n", mStartingPos);
    switch (mStartingPos) {
    case LEFT:
    case RIGHT:
...
//      mCommands.add(new DriveStraight(mDriveTrain, mData, AutoDimensions.SAME_SIDE_SWITCH_TO_NULL_ZONE));
	...
	public void doExchange() {
		// TODO replace with turning scalar
		log.info("Doing exchange autonomous starting on %s\n", mStartingPos);
		switch (mStartingPos) {
		case LEFT:
			break;
...
//    if(mStartingPos != EStartingPosition.LEFT) mStartingPos = EStartingPosition.LEFT;
	...
	public boolean isCubeActionOnMySide(ECubeAction c) {
	  log.info("Cube Action: %s Scale Side: %s Switch Side: %s\n", c, mScaleSide, mSwitchSide);
		switch (c) {
		case EXCHANGE:
    ...
    case SWITCH:
      log.info("Switch auto availability: " + isCubeActionOnMySide(c));
