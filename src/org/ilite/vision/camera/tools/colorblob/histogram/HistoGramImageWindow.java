...
public class HistoGramImageWindow extends ImageWindow {
	...
	histImage = calcHistogramSingleChannel(Arrays.asList(rgb.get(0)),histImage,histSize, new Scalar(255,0,0));
	histImage = calcHistogramSingleChannel(Arrays.asList(rgb.get(1)),histImage,histSize, new Scalar(0,255,0));
	histImage = calcHistogramSingleChannel(Arrays.asList(rgb.get(2)),histImage,histSize, new Scalar(0,0,255));

	super.updateImage(OpenCVUtils.toBufferedImage(histImage));
    }

    private Mat calcHistogramSingleChannel(List<Mat> rgb, Mat pHistImage, MatOfInt pHistSize, Scalar pHistoColor) {
	//Calculate histogram
	...
	{                   
	    Core.line(
		    ...
		    new org.opencv.core.Point( i, pHistImage.rows()-Math.round( histogram.get(i,0)[0] )) ,
		    pHistoColor,
		    1, 8, 0 );
	}
	return pHistImage;
